...
def populate_checkout_fixture(
    authenticated_client: TestClient, populate_book, populate_patron
):
...
def test_checkout_operation(authenticated_client: TestClient, populate_checkout):
    response = authenticated_client.post(
        "/operations/checkout/", json=TEST_CHECKOUTS[2]
    )
    data = response.json()
...
def test_return_operation(authenticated_client: TestClient, populate_checkout):
    response = authenticated_client.post(
        "/operations/return/", json={"patron_id": 1, "book_id": 1}
    )
    data = response.json()
